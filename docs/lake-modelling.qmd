...
![Schematic illustration of the TPKALK box model for lake calcium concentration for a single lake under the assumption of fixed water volume. Symbols are defined in the text.](./images/box_model.png){#fig-box-model}{width=400}
...
This suggests there may be a better way of accounting for organic matter concentration than using $A_{pH}$ in @eq-ph-ca?
...
Based on this, it sounds as though we might be able to treat all lime products the same in terms of long-term dissolution? This could correspond to using a single $K_L$ value for all lime types, although this will also mean that medium term dissolution rates (i.e. the first lake-bottom lime to dissolve) will also be the same for all limes. Thoughts?
:::
