...
NV is usually specified in percent: pure calcium carbonate has an NV of 100 %, pure magnesium carbonate has a NV of 119 %, and typical dolomite products have NVs that are somewhere in between. For the same mass, products with a higher NV will be more effective at increasing pH, *as long as all the lime dissolves*. In practice, different limes exhibit different dissolution rates, which must also be accounted for.
 ...
 * $C_{bott} = F_{active} (D - C_{inst})$ is the amount of Ca (in mg/l) that sinks to the lake bottom and remains active for dissolution. Note that $(D - C_{inst})$ is the total amount of lime that sinks to the bottom, and $F_{active}$ is the proportion of this that remains "active". Typical values are around 0.4, based on the observation that about 60% of the lime arriving on the lake bottom becomes "deactivated" and never dissolves.
...
The model defined above is capable of simulating how pH and Ca concentration (and Mg, if relevant) will change over time for a single lake. The model requires the following iinputs:
...
To simplify the process of comparing different lime products, it is common to consider a **standard lake** with a typical **discharge profile**.
 ...
 * **Discharge:** *If the three parameters above are fixed, then the discharge is pre-determined and must be held constant. However, it has also been suggested that we (i) should use a typical flow profile that varies by month, in which case the lake will also have a set of monthly residence times, and (ii) tune both the discharge and mean depth to achieve different residence times. In both these cases, it appears that mean depth and residence time are not actually fixed characteristics of the standard lake?*

 * **pH before liming:** 4.0, 4.5, 5.0, 5.5 and 6.0. *OK, but given the range of variation I suggest treating this as a variable rather than "standard characteristic".*
 ...
 * **Andel av innsjøoverflate som kalkes:** 50%. *I'm not sure what effect this should have, since the "dose" is defined in mg/l based on the volume of the whole lake. In the context of the TPKALK model, what effect should this parameter have on the calculations?*

 * **CaCO3-ekv:** 107% (Microdol 5). *OK, but presumably this isn't a fixed parameter for the "standard lake", as I thought the whole purpose was to compare different lime types?*

 * **Application method:** Wet spreading from boat. *I think. OK if so.*

In addition, the following characteristics have not been mentioned, but seem to be required for a standard lake:

 * $pH_0$ and $A_{pH}$
 * Initial Ca (and Mg, if relevant) concentrations in the lake (or are these estimated some other way?)
:::

::: {.callout-important}
**Some initial comments from Atle:**
...
Is this the same equation as described above in @sec-inst-diss (@eq-eq-cor-depth), or is Atle referring to an alternative? An equation that directly estimated the change in instantaneous dissolution would be more practical than @eq-cor-depth (which instead modifies the pH values used in a lookup table).
...
This suggests that there may be a better way of accounting for organic matter concentration than using $A_{pH}$ in @eq-eq-ph-ca?
...
Based on this, it sounds as though we might be able to treat all lime products the same in terms of long-term dissolution. This could correspond to using a single $K_L$ value for all lime types, although this will also mean that medium term dissolution rates (i.e. the first lake-bottom lime to dissolve) will also be the same. Thoughts?
:::
