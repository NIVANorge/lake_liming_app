...
The integral in @eq-inst-diss can be approximated from the measured data (@tbl-instantaneous-dissolution) using e.g. [Simpson's rule](https://en.wikipedia.org/wiki/Simpson%27s_rule) or the [trapezoidal rule](https://en.wikipedia.org/wiki/Trapezoidal_rule). By default, the application uses the trapezoidal rule.
