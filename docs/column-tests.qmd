...
@tbl-instantaneous-dissolution is used to estimate the total amount of Ca that has dissolved in each column after 16 hours. It is assumed that each column is horizontally well-mixed and that concentrations only vary with depth, $x$. If this relationship is perfectly known for all depths in the column, the total amount of Ca between depths $x_1$ and $x_2$ could be found by the integral

$$
M_{Ca} = A \int_{x_1}^{x_2} C(x) \ dx
$$ {#eq-conc-depth-integral}

The overall proportion of the available Ca that has dissolved is called the **instantaneous dissolution** and be obtained by dividing {#eq-conc-depth-integral} by {#eq-evenly-mixed} evaluated over the entire column (typically from 0 to 2 m depth)

$$
D_{inst} = \frac{\int_{0}^{2} C(x) \ dx}{10 F_{Ca} (200 - 0)}
$$ {#eq-inst-diss}
